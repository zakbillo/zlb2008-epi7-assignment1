...
We are interested in knowing to what extent participation in the NSW Demonstration program affects future income among Black participants. In causal phrasing: if, perhaps contrary to reality, all eligible Black individuals received the NSW Demonstration training program, what would their expected real earnings in 1978 be compared to if, perhaps contrary to reality, none of them received the program?
...
\item Marginal Exchangeability: $Y^a \perp A$. All common causes of the job training assignment and 1978 earnings are controlled for by random treatment assignment.

\item Positivity: $0 < p(A=1 \mid W) < 1$ for observed strata $W$. Every individual must have a non-zero probability of receiving or not receiving the training. 
...
\item $E[Y^1|Black=1] = E[E[Y^1|Black=1]]$ (by the law of iterated expectations)
\item $=E[E[Y^1|A = 1,Black =1]]$ (by marginal exchangeability, $Y^a \perp A$) 
\item $= E[E[Y|A = 1,Black =1]]$ (by consistency)

\end{itemize}

We can replace the 1's with 0's to find $E[Y^0|Black=1]$. Thus, the causal ATE is identified by the statistical estimand: $$\hat{ATE} = \mathbb{E}[\mathbb{E}[Y \mid A=1, Black = 1] - \mathbb{E}[Y \mid A=0, Black = 1]]$$
...
g1 <- pmax(0.025, pmin(0.975, g1)) # Trunate for better positivity
...
G-computation returns a causal $\hat{ATE}$ of \$1371.454 (95% CI: -281.69, 3024.59),
AIPTW returns a causal $\hat{ATE}$ of \$1417.464 (95% CI: -408.16, 3243.09), and TMLE + SL returns a causal $\hat{ATE}$ of \$873.432 (95% CI: -582.24, 2329.11). 
