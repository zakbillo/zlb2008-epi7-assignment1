...
\item $W$ is a set of 8 baseline covariates that could confound the effect of $A$ on $Y$: age, years of education, indicators for Black and Hispanic race, marital status, absence of a high school degree, and real earnings in 1974 and 1975. The specified categorical variables in this dataset have been factored for analysis.
...
Our sample is comprised of 445 participants, corresponding to 185 treated and 260 control subjects, and 10 the aforementioned variables. 

```{r}
data(lalonde)

# Let's make sure the binary variables are coded correctly 
cat_vars <- c("black", "hispanic", "married", "nodegree")
lalonde[cat_vars] <- lapply(lalonde[cat_vars], function(x) {
 factor(x, 
        levels = c(0, 1),
        labels = c("No", "Yes"))
})
...
W <- lalonde[, c("age", "education", "black", "hispanic", "married", "nodegree", "re74", "re75")]
...
Our target parameter is the Average Treatment Effect (ATE), defined on the counterfactual outcomes as $\mathbb{E}[Y^{a=1} - Y^{a=0}]$. This represents the difference in expected 1978 earnings where $Y^{1}$ is the counterfactual outcome if one, perhaps contrary to reality, recieved job training through NSW and $Y^{0}$ is the counterfactual outcome if one, perhaps contrary to reality, did not receive job training (control). 

To identify the causal ATE from our observational data, we must meet three assumptions: 

\begin{itemize}

\item Consistency: An individual's observed outcome $Y$ when they receive treatment $A=a$ is equal to their counterfactual outcome $Y^a$.

\item Conditional Exchangeability: $Y^a \perp A \mid W$. All common causes of the job training assignment and 1978 earnings are captured in covariate vector $W$.

\item Positivity: $0 < p(A=1 \mid W) < 1$ for all strata of $W$. Every individual must have a non-zero probability of receiving or not receiving the training. 
...
for(i in 1:B) {
  ...
  boot_model <- glm(re78 ~ treatment + age + education + black + hispanic + married + nodegree + re74 + re75, 
                    data = boot_data, family = gaussian())
...
ps_model <- glm(treatment ~ age + education + black + hispanic + married + nodegree + re74 + re75, 
                data = lalonde, family = binomial())

# Get the predicted probabilities
g1 <- predict(ps_model, type = "response") 
g1 <- pmax(0.025, pmin(0.975, g1)) # Predicted probability of being treated for each individual. Truncated due to positivty violations.
...
lalonde <- lalonde[, c("age", "education", "black", "hispanic", "married", 
                          "nodegree", "re74", "re75", "treatment", "re78")]
# Define the SuperLearner library
sl_lib <- c("SL.glm", "SL.mean", "SL.gam", "SL.earth", "SL.glmnet", "SL.xgboost")
...
G-computation returns a causal $\hat{ATE}$ of \$1,514.24 (95% CI: 121.56, 2974.85). If unmeasured confounding, conditional exchangeability, consistency and positivity hold, this implies enrollment in the NSW Demonstration increased real earnings in 1978 by \$1,514.24 more than those who weren't enrolled in the program. However, we notice that the both the causal $\hat{ATE}$ of AIPTW and TMLE + SL indicate positive earnings for enrolled participants, but their confidence intervals suggest the difference in real earnings in 1978 is due to chance. 

# Conclusion

Under the positivity, conditional exchangeability, and positivity assumptions, we were unable to identify the causal $\hat{ATE}$ for the effect of NSW Demonstration enrollment on real earnings in 1978 a small sample of U.S. adults. While G-computation appears to show increases in earnings with program enrollment, both the doubly-robust AIPTW and TMLE + SL etimators suggest this finding is due to chance. This may be due to low predicted treatment probabilities for certain covariate combinations, or the fact that we do not have access to all reasonable confounders of the effect. However, in this short analysis, we found a no significant effect of NSW Demonstration enrollment on real earnings in 1978.
